...
angular.module('bootstrapLightbox').provider('Lightbox', function () {
  ...
  this.getThumbUrl = function (image) {
    return typeof image === 'string' ? image : image.url;
  };
  this.getImageThumb = function (image) {
    return typeof image === 'string' ? image : image.thumb;
  };
  this.getSources = function (image) {
    return [typeof image === 'string' ? {file: image} : {file: image.url}];
  };
  this.isPlayable = function (image) {
    return !!image;
  };
   ...
   * @memberOf bootstrapLightbox.Lightbox
   */
  this.isDocument = function (image) {
    if (typeof image === 'object' && image && image.type) {
  ...
  this.isSharedVideo = function (image) {
    return image && this.isVideo(image) &&
   ...
   * @memberOf bootstrapLightbox.Lightbox
   */
  this.isAudio = function (image) {
    return this.getImageUrl(image).match(/\.(mp3)$/);
  };

  this.getVideoOptions = function(image){
    return {
      //file: this.getImageUrl(image),
      //image: this.getImageThumb(image),
  ...
  this.getAudioOptions = function(image){
    return {
      //file: this.getImageUrl(image),
      sources: this.getSources(image),
      image: this.getImageThumb(image),
  ...
  this.getDocOptions = function(image){
    return {
      //file: this.getImageUrl(image),
      sources: this.getSources(image),
      width: '100%',
  ...
  this.$get = ['$document', '$injector', '$uibModal', '$timeout', 'ImageLoader', '$http', 'currentBaseUrl',
      function ($document, $injector, $uibModal, $timeout, ImageLoader, $http, currentBaseUrl) {
    // optional dependency
    ...
    Lightbox.getImageVersion = this.getImageVersion;
    Lightbox.getThumbUrl = this.getThumbUrl;
    Lightbox.getSources = this.getSources;
    Lightbox.isPlayable = this.isPlayable;
    Lightbox.getImageCaption = this.getImageCaption;
    Lightbox.getImageThumb = this.getImageThumb;
    Lightbox.calculateImageDimensionLimits = this.calculateImageDimensionLimits;
    Lightbox.calculateModalDimensions = this.calculateModalDimensions;
    Lightbox.isVideo = this.isVideo;
    Lightbox.isImage = this.isImage;
    Lightbox.isAudio = this.isAudio;
    Lightbox.isDocument = this.isDocument;
    Lightbox.isSharedVideo = this.isSharedVideo;
    Lightbox.getVideoOptions = this.getVideoOptions;
    Lightbox.getAudioOptions = this.getAudioOptions;
    Lightbox.getDocOptions = this.getDocOptions;
    ...
    Lightbox.openModal = function (newImages, newIndex, modalParams) {
      ...
      Lightbox.modalInstance = $uibModal.open(angular.extend({
        ...
        'controller': ['$scope','$sce','field', function ($scope, $sce, field) {
    ...
    Lightbox.setImage = function (newIndex) {
      ...
      var version = Lightbox.getImageVersion(image);
      if(version && angular.isDefined(version.sizes) && version.sizes !== null) {
          var sizes = version.sizes.split("x");
          var imageWidth = parseInt(sizes[0]);
          var imageHeight = parseInt(sizes[1]);
      }

      var imageThumb = image.thumb;
      ...
      var success = function (properties) {
        ...
        var version = Lightbox.getImageVersion(Lightbox.image);
        if(version && angular.isDefined(version.sizes) && version.sizes !== null) {
            var sizes = version.sizes.split("x");
            Lightbox.imageVersion = {
                url: properties.imageUrl || imageUrl,
                width: parseInt(sizes[0]) || imageWidth,
                height: parseInt(sizes[1]) || imageHeight,
                thumb: Lightbox.image.thumb || imageThumb
            };
        }
        Lightbox.videoOptions = Lightbox.getVideoOptions(image);
        Lightbox.audioOptions = Lightbox.getAudioOptions(image);
        Lightbox.docOptions = Lightbox.getDocOptions(image);
      ...
      if (!Lightbox.isVideo(image) && !Lightbox.isAudio(image)) {
    ...
    Lightbox.showVideo = function(version){
      return version.url;
    };

    Lightbox.autoNext = false;

    Lightbox.edit = [];
    Lightbox.reEncode = [];
    Lightbox.showReencode = [];
    Lightbox.reEncodeInQueue = false;

    Lightbox.ReEncodeFile = function(file, params){
      ...
      if(param){
        $http.post(currentBaseUrl + 'reEncode', {id: file.id, param: param}).success(function (data) {
          Lightbox.reEncodeInQueue = true;
          Lightbox.showReencode[file.id] = data;
          Lightbox.edit[Lightbox.index] = null;
        });
      }
      Lightbox.reEncode = [];
    };
    Lightbox.CreateNewVersion = function(file, format){
      ...
      Lightbox.reEncodeQueue = function(file){
      var encodeStatus = false;
      var ReEncode = 1;
      var encode = 2;
      if(file !== undefined && file.contest_file_versions !== undefined) {
          angular.forEach(file.contest_file_versions, function (item) {
              if (item.status == 0 && item.config) {
                  encodeStatus = ReEncode;
              } else {
                  if (item.status == 0) encodeStatus = encode;
              }
          });
      }
      return encodeStatus;
    ...
    Lightbox.changeStatus = function (file, status, autoNext) {
