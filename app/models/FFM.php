 ...
 *
 * Examples
 * Converted from FLAC to AAC:
 *      FFM::convert()->input('foo.flac')->bitrate(128)->output('bar.aac')->go();
 *
 * Convert video:
 *      FFM::convert()->input('foo.avi')->bitrate(300, 'video')->output('bar.flv')->go();
 *
 * FFM mediainfo:
 *      FFM::getMediaInfo('foo.mov');
 *
 * FFM thumbs:
 *      FFM::getThumbnails('foo.mp4', 'foo-thumb', 5); // Yields 5 thumbnails
 *
 * FFM thumbnify (short preview)
 *      FFM::thumbnify('foo.mp4', 'foo-thumb', 40, '400'); // 40 thumbnails (every 10 seconds) and video has 400 secs
 *
 * FFM have preset but we can pass out own
 *      FFM::convert()->input('foo.flac')->output('bar.mp3')->go('-b:a 64k -ac 1');
 *
 * Tracking progress (NOT TESTED)
 ...
 *      FFM::convert()->input('foo.avi')->output('bar.mp3')->progress('uniqueid')->go();
 *      public function getJobProgress($id) {
 *          return FFM::getProgress($id);
 *      }
 */
class FFM {
    /**
    ...
    public static function getConverterVersion() {
        ...
        $output = preg_match("/ffmpeg version (?P<major>[0-9]{0,3}).(?P<minor>[0-9]{0,3}).(?P<revision>[0-9]{0,3})/", $output, $parsed);
        // Verify output
     ...
     * @return boolean
     */
    public static function getThumbnails($input, $output, $count = 5, $format='jpg') {
        // Round user input
        $count = round($count);
        ...
        $command = self::getConverterPath().' -i '.$input.' -vf "select=gt(scene\,0.5)" -frames:v '.$count.' -f image2 -vsync vfr '.$output.'-%04d.'.$format." >> /tmp/error-output.txt 2>&1 &";
        passthru("echo 'COMMAND: ".$command."' >> /tmp/error-output.txt 2>&1 &");
        passthru($command);
        return true;
    }
     ...
     * @return boolean
     */
    public static function thumbnify($input, $output, $count=5, $duration, $format='jpg') {
        ...
        // Execute thumbnail generator command
        $command = self::getConverterPath().' -i '.$input.' -vf fps=fps=1/'.$total.' '.$output.'-%04d.'.$format;
    ...
    /**
     * Genera un archivo temporal para poder sacar el waveform
     * @param string $input
     * @param string $output
     */
    public static function generateWaveform($input, $output) {
        $temp = "/tmp/temp-waveform-".date('Y.m.d.H.i.s').microtime(true).".wav";
        $command = self::getConverterPath().' -y -i '.$input.' '.$temp;
        passthru("echo 'COMAND: ".$command."' >> /tmp/error-output.txt 2>&1 &");
        passthru($command.' >> /tmp/error-output.txt 2>&1');
        $fs = new Filesystem();
        if ($fs->exists($temp)) {
            $AF = new AudioWaveform();
            $AF->loadFile($temp);
            if ($AF->wave_id == "RIFF"){
                $AF->visual_graph_color="#000000";
                $AF->visual_background_color="#FFFFFF";
                $AF->visual_grid=false;
                $AF->visual_border=false;
                $AF->visual_graph_mode=0;
                $AF->visual_width=400;
                $AF->visual_height=400;
                $AF->visual_fileformat="jpeg";
                $AF->getVisualization($output);
            } else {
                passthru("echo 'No le puedo sacar el waveform.' >> /tmp/error-output.txt &");
            }
            $fs->delete($temp);
        }
    }
    ...
    public function go($arg = null) {
        ...
        if (Config::get('ffm.progress') === true) {
            // Get temp dir
